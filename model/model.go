...
func (m Model) exportSnapshot() export.SnapshotRecord {
	...
	if p := m.PromSnapshot; p != nil && p.Err == nil {
		app := &export.AppMetrics{
			...
			SMSFailTotal:                  p.SMSFailTotal,
			SMSSuccessTotal:               p.SMSSuccessTotal,
			SMSAliOK:                      p.SMSAliOK,
			SMSTencentOK:                  p.SMSTencentOK,
			SMSAliBusinessStopped:         p.SMSAliBusinessStopped,
			SMSTencentPhoneFormat:         p.SMSTencentPhoneFormat,
			SMSTencentInsufficientBalance: p.SMSTencentInsufficientBalance,
			SMSNoProviderSuccess:          p.SMSNoProviderSuccess,
			SMSOtherFailure:               p.SMSOtherFailure,
